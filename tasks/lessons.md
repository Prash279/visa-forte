...
**Lesson 7 — A JSON rules table is only a source of truth if every tier boundary in it was read off the official grid, not inferred from the key name**
- **What went wrong:** The Assessment page over-scored applicants by up to 25 CRS points. `crs-rules.json` held a Section C skill-transferability key named `bachelorsPlus` worth 25/50, and `crs-calculator.ts` routed `bachelors`, `two_or_more_degrees`, `masters` and `doctoral` into it. canada.ca's grid has **three** education tiers, not two: a single bachelor's is "post-secondary credential of one year or longer" (13/25), and the 25/50 tier requires two or more credentials with one of three years or longer, a master's, or a doctorate. The engine's own ground-truth test suite never caught it because every fixture profile held a Master's — which lands in the correct tier either way. Seven further defects sat alongside it in the same file: no per-group 50 cap in Section C, no second-official-language combined cap in Section A, an FSW work-experience boundary one year early, FSW second language scored per-ability instead of all-or-nothing, the FSW age table truncated at 42, own-Canadian-work adaptability at 5 instead of 10, spouse adaptability elements ignored entirely, CEC's language bar hardcoded at CLB 7 when TEER 2/3 needs only CLB 5, and FST hardcoded `eligible: false` with a fixed "NOC TEER 1" message shown to TEER 2 applicants.
- **Why it happened:** The rules file was treated as verified because it *existed* and carried a `meta.lastVerified` date. In practice the date attested to the file, not to each row. A key called `bachelorsPlus` encodes an assumption — "bachelor's and everything above it behave the same" — that reads as obviously true and is false on the actual grid. The test fixtures were written from the engine's own output rather than from the canada.ca tables, so they locked the bug in place instead of exposing it. One engine, four consumer surfaces (Assessment page, CRS Modeller, CanVisa Pro, CanVisa Lite), so a single wrong tier was wrong everywhere at once.
- **The rule going forward:** Every numeric row in `crs-rules.json` must be traceable to a *cell* on the canada.ca grid, and the JSON key must name the grid's own category — never a shorthand that groups tiers the grid keeps apart. `meta.lastVerified` attests to a full row-by-row pass, not a spot check; if only part of the file was re-read, say which part. Test fixtures for a scoring engine must be built from the published tables and must include at least one profile that sits at each tier **boundary** — a fixture set where every profile is comfortably inside one tier proves nothing. And never hardcode an eligibility verdict (`const fstEligible = false`) with a prose reason that assumes a fact about the applicant the profile already contradicts.
