...
**Lesson 6 — A "verified against official source" claim must cite the authority the user's regulator points to, not the source that was convenient to fetch**
- **What went wrong:** The NOC verifier's live check ran against Statistics Canada's IMDB page (inherited from the admin classifier, chosen because it is lightweight server-rendered HTML) and the UI told applicants "verified against Statistics Canada." IRCC's own "Find your NOC" instruction points applicants to noc.esdc.gc.ca — so a client-facing immigration tool citing StatCan as its verification authority read as wrong to the owner within hours, even though both sites publish the identical NOC 2021 classification and no incorrect data was shown.
- **Why it happened:** An internal engineering convenience (which page is easiest to verify programmatically) leaked into user-facing trust copy. Nobody asked "which source would the applicant's officer or IRCC name?" when the badge text was written.
- **The rule going forward:** For any user-facing "verified against X" claim, X must be the source the governing authority directs the end user to (for NOC: ESDC's noc.esdc.gc.ca; for programme rules: canada.ca). A technically-equivalent mirror may be used as a *fallback* check, but then the UI must name the actual source that confirmed. Verification plumbing may be pragmatic; verification *claims* must follow the user's authority chain.

**Lesson 7 — A degraded mode must drop its ranking language too, not just add a caution banner**
- **What went wrong:** The duties-based NOC matcher's `lexicalFallback` handed TF-IDF hit #1 the band `'strongest'`, so the degraded response rendered "**Strongest match** — NOC 21300 Civil Engineers" with the `nocp-card--top` highlight for data-science duties, immediately below an amber banner saying the results were keyword matches only and could be wrong. Both statements were on screen at once. The banner satisfied the letter of Lesson 5's "label degraded mode as degraded" while the card contradicted it and put the visual emphasis on the wrong NOC code — the exact code an applicant might copy onto a PR application.
- **Why it happened:** The caution banner was treated as the whole of the degradation work, so the *data shape* was never revisited. `'strongest' | 'review'` is a ranking vocabulary that only the Claude stage earns the right to use; the fallback reused it unchanged because it was the existing type, and keyword order silently passed itself off as a ranking. It surfaced only because an end-to-end probe hit the live route with no `ANTHROPIC_API_KEY` present — the unit tests never exercised the degraded response's band assignment.
- **The rule going forward:** When a pipeline degrades, strip the confidence vocabulary from the *payload*, not just from the surrounding copy — if the ranking stage did not run, nothing may be returned as "strongest", "best", "top", or carry a highlight style. Ordering that comes from a recall score is not a ranking and must not be presented as one. And probe the degraded path deliberately (unset the key, force the fallback) rather than assuming it is correct because the happy path is: the fallback is precisely the branch that ships unexercised.

**Lesson 8 — Before merging a branch into `main`, check whether `main` moved without you, not just whether your branch moved**
- **What went wrong:** After a long session of work on `feat/noc-duties-search`, I merged it into `main` by fast-forwarding straight to the feature branch's tip. But `origin/main` had quietly moved ahead in the meantime — two pull requests had already been merged there, plus a string of automated data-update commits from a scheduled workflow — and my local `main` was a stale copy that had never picked any of that up. The fast-forward succeeded (git didn't stop me), but it would have pushed a version of `main` that silently dropped both merged PRs and every automated commit the moment I ran `git push`.
- **Why it happened:** I checked that my *feature branch* had the right commits, but never checked whether `origin/main` itself had changed since I last looked at it. A fast-forward merge only fails if history conflicts — it says nothing about whether the branch you're merging *into* is still the branch you think it is. I caught it by re-reading the merge output narration rather than trusting the merge's "success," and had not yet pushed.
- **The rule going forward:** Before merging any feature branch into `main`, run a fetch and directly compare `main` against `origin/main` in both directions (commits main has that origin doesn't, and commits origin has that main doesn't) — not just the feature branch against `main`. If `origin/main` has moved, merge `origin/main` into the feature branch first (a safe, reversible step — it just adds a merge commit) and only then fast-forward `main` onto the reconciled result. Never push to `main` on the strength of a successful merge alone; confirm the branch relationship first.
...
**Lesson 7 — An eval that scores a different model than production measures nothing**
- **What went wrong:** `MODEL = 'claude-sonnet-5'` was declared separately in both NOC routes, and the new eval was about to declare a third copy. Three copies drift; the day one route moves, the eval reports a number about a model nobody runs.
- **Why it happened:** each route was written at a different time and each looked self-contained.
- **The rule going forward:** anything the eval must hold identical to production (model, max_tokens, the user-message template) is defined once in `noc-classify.ts` and imported. If the eval builds its own copy of the prompt, it is testing itself.

**Lesson 8 — On an adaptive-thinking model, max_tokens is not the answer's size**
- **What went wrong:** the golden-corpus eval returned `unparseable model reply` on 2 of 10 cases. `stop_reason=max_tokens`: the JSON was cut mid-string, `extractJsonObject` found no balanced brace and returned null, so the WHOLE classification was thrown away — including a correctly-ranked top code that had arrived complete. Production impact: silent keyword fallback on the public tool, 503 on a paid admin file.
- **The first diagnosis was wrong, and that is the lesson.** I read it as "rationales too long", capped them at 40 words and raised 4096→8192. A different case then failed the same way. Measuring one case three times on IDENTICAL input showed the visible reply was ~120 tokens while `output_tokens` came back 4151, 4176, then 7938. Sonnet 5 thinks adaptively and those thinking tokens bill against `max_tokens`. The ceiling was never sized for the answer — it has to cover the answer PLUS however much the model decides to think that run, which varies and which the caller cannot control.
- **The rule going forward:** on any adaptive-thinking model, size `max_tokens` for thinking variance, not for the visible output — a small answer is no evidence that a small ceiling is safe. Always record `stop_reason` and `output_tokens` in any eval that calls a model; not having them cost two throwaway probe scripts to rediscover something one logged field would have shown immediately. And keep a salvage path regardless: whenever a parse failure discards a whole response, ask what arrived intact before throwing it away.

**Lesson 9 — The most common occupation in the pool had no anchor, and the shortlist inflated it**
- **What went wrong:** for a plain backend developer's duties the PUBLIC top 30 contained exactly one plausible code — 21231 Software engineers and designers, at rank 1. That is the ARCHITECTURE code, the one thing those duties excluded. 21230 and 21234 were absent entirely (ranks 35 and 57 of 60) and 21232 sat at 13, while ranks 2-8 were Civil, Chemical, Mechanical, Computer, Electrical, Industrial and Metallurgical engineers.
- **Why it happened:** the existing anchor only fired on data-science and ML vocabulary. Ordinary software vocabulary (TypeScript, REST API, pull request, CI/CD) appears nowhere in NOC 2021 text, while generic engineering words do — so the classifier was handed a shortlist whose only software option was one TEER-level too senior.
- **The rule going forward:** an anchor's job is to make the RIGHT CHOICE AVAILABLE, and the failure mode to hunt for is not "correct code absent" but "only a wrong code present". Check what the shortlist offers, not just whether the answer is somewhere on it. Measured before/after ranks belong in the anchor comment.

**Lesson 10 — A corpus case with two defensible answers measures opinion, not accuracy**
- **What went wrong:** the software-developer case was labelled 21232, then corrected to 21230 after the model argued the official text convincingly — and then the next run returned 21232 again on identical input. Two runs, two answers, both defensible.
- **Why it happened:** NOC 2021 does not cleanly separate 21230 from 21232. Their lead statements overlap almost entirely; 21232's duty list leans to game and multimedia work while its example titles include "software developer". The source points both ways, so asserting one was asserting my preference.
- **The rule going forward:** when the official text genuinely does not separate two codes, record both as acceptable and keep asserting the thing that DOES matter — here, that neither run drifted to the architecture code 21231. Never quietly widen an assertion to make a red test green; widen it only when the source itself is ambiguous, and write down why. Both codes were TEER 1, so the ambiguity changed nothing for the client.

**Lesson 11 — A test can fail because its premise expired, not because the code broke**
- **What went wrong:** `leaves ordering alone when no anchor fires` used software duties as its example of unanchored input. Adding a software anchor made that input anchored, and the test went red while the behaviour it guards was perfectly correct.
- **Why it happened:** the fixture encoded an assumption ("nothing anchors software") that the same commit invalidated.
- **The rule going forward:** when a test fails, first ask whether it is asserting the behaviour or asserting a fact about the fixture. Fix the fixture, never the assertion — and leave a comment naming the assumption so the next person does not re-break it.

**Lesson 12 — UI hint text is documentation, and it goes stale like any other**
- **What went wrong:** the CanVisa Pro duties field told the consultant "If you set the NOC Code field above, the assessment uses your code instead of classifying - your code always wins." `generatePnp()` does the opposite: the classifier always runs when duties are present, the manual code is passed only as a hint, and the classifier result wins.
- **Why it happened:** the behaviour was changed to classifier-wins and the hint beneath the field was not part of the diff.
- **The rule going forward:** hint text that describes precedence, defaults or "what wins" is behavioural documentation sitting inside the UI. When the rule changes, grep the JSX for the sentence that stated the old one. On an admin tool that prepares client files, a wrong hint is worse than no hint.