...
**Lesson 5 — The `--nav-h` CSS custom property is silently stripped from the compiled `:root` block; never rely on it**
- **What went wrong:** A resources page fix used `scroll-margin-top: var(--nav-h)` to stop the fixed nav from covering the `#tools` section on scroll. It compiled with no errors and looked correct in the source file, but did nothing live — `getComputedStyle(document.documentElement).getPropertyValue('--nav-h')` returned an empty string in the browser. Every other variable declared in the same `:root` block (`--prussian`, `--sand`, `--max-w`, `--text-w`) survived the build; only `--nav-h` vanished. This means the existing `.asx-result-view { padding-top: var(--nav-h); }` rule on the assessment page (shipped earlier as a nav-overlap fix) has been silently broken the same way — it currently resolves to nothing, not 81.4px.
- **Why it happened:** Not fully diagnosed — most likely Turbopack/Lightning CSS's custom-property handling drops `--nav-h` specifically because its only other declaration lives inside a `@media (max-width: 860px) { :root { ... } }` block (globals.css:497), and the two conflicting `:root` declarations for the same property across a plain rule and a media-scoped rule confuse the optimizer's dead-code elimination for CSS variables. This was only caught by directly inspecting `document.styleSheets` in a live browser — reading the source CSS file gave no indication anything was wrong.
- **The rule going forward:** Do not introduce new `var(--nav-h)` usages until this is root-caused and fixed at the source (or the variable is redeclared as a single non-conditional value). For any new fixed-nav-clearance CSS (`scroll-margin-top`, `padding-top`, etc.), use a literal pixel value instead (e.g. `90px`, safely above both the 81.4px desktop and 69.8px tablet+ nav heights) and verify the computed style in an actual browser (`getComputedStyle(el).propertyName`), not just by reading the CSS source — this class of bug produces a clean build with a silently no-op runtime effect.
