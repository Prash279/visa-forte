  ...
  return (
    <div className="booking-calendar">
      ...
      <div className="booking-cal-nav">
        <button className="booking-cal-nav-btn" onClick={prevMonth}>
          ← Prev
        </button>
        <span className="booking-cal-month-label">
          {MONTH_NAMES[month]} {year}
        </span>
        <button className="booking-cal-nav-btn" onClick={nextMonth}>
          Next →
        </button>
      </div>
      ...
      <div className="booking-cal-grid">
        ...
        {DOW.map((d) => (
          <div key={d} className="booking-cal-dow">
            {d}
          </div>
        ))}
        ...
        {days.map((day, i) => {
          if (day === null) {
            return (
              <div
                key={`pad-${i}`}
                className="booking-cal-cell booking-cal-cell-empty"
              />
            );
          }
          ...
          return (
            <div
              ...
              className={cellClass}
              onClick={() =>
                hasBookings && setSelectedDate(isSelected ? null : dateStr)
              }
        ...
        <div className="booking-cal-detail">
          <p className="booking-cal-detail-heading">
            {selectedBookings.length === 0
              ...
              : `${selectedBookings.length} booking${selectedBookings.length !== 1 ? 's' : ''} — ${new Date(
                ...
                return (
                  <div key={b.id} className="booking-cal-detail-row">
                    <div className="booking-cal-detail-client">
                      ...
                      <a
                        href={`mailto:${b.email}`}
                        className="booking-cal-detail-email"
                      >
                        {b.email}
                      </a>
                    </div>
                    <span className="booking-cal-detail-service">
                      {b.serviceTier}
                    </span>
                    <div className="booking-cal-detail-meta">
                      <span
                        className={`admin-badge ${b.paymentStatus === 'paid' ? 'admin-badge-paid' : 'admin-badge-new'}`}
                      >
                        {b.paymentStatus}
                      </span>
                      <span className="booking-cal-detail-amount">
                        {amount}
                      </span>
                    </div>
                  </div>
