...
import {
  analyzeSinpDraws,
  type SinpDrawAnalysis,
  type SinpDrawVerdict,
} from '@/lib/sinp-draws';
...
function DifficultyChips({
  ...
  stream: PnpStreamMatch['stream'];
}): React.JSX.Element | null {
  const tags: Array<DifficultyTag | 'employer_required'> = [
    ...(stream.difficultyTags ?? []),
    ...(stream.criteria.jobOfferRequired === 'required'
      ? (['employer_required'] as const)
      : []),
  ];
  ...
  return (
    <div className="pnp-difficulty-tags">
      {tags.map((t) => (
        <span
          key={t}
          className={`pnp-difficulty-tag pnp-difficulty-tag--${t.replaceAll('_', '-')}`}
        >
          {DIFFICULTY_LABEL[t]}
        </span>
      ))}
  ...
  const rows: { label: string; value: number; max: number }[] = [
    {
      ...
      value: m.scoreBreakdown.matchStrength,
      max: 40,
    },
    {
      label: 'Strategic value',
      value: m.scoreBreakdown.strategicValue,
      max: 30,
    },
    { label: 'Stream status', value: m.scoreBreakdown.openStatus, max: 20 },
    {
      label: 'Processing speed',
      value: m.scoreBreakdown.processingSpeed,
      max: 10,
    },
  ];
  return (
    ...
    <div className="pnp-elig">
      ...
      {checks.map((c) => (
        <div key={c.label} className="pnp-elig-row">
          <span
            className={`pnp-elig-dot pnp-elig-dot--${c.status}`}
            aria-hidden="true"
          />
          <span className="pnp-elig-label">{c.label}</span>
          <span className={`pnp-elig-val pnp-elig-val--${c.status}`}>
            {c.applicant}
          </span>
          <span className="pnp-elig-req">
            {c.requirementKind === 'binary'
              ? c.requirement
              : `requires ${c.requirement}`}
          </span>
        </div>
      ))}
    </div>
...
function nextMove(m: PnpStreamMatch): string {
  if (m.verdict === 'confirmed')
    return 'You meet every checkable requirement — proceed to documentation.';
...
function StreamCardExtras({ m }: { m: PnpStreamMatch }): React.JSX.Element {
  ...
  const cond = m.eligibilityChecks.filter(
    (c) => c.status === 'conditional',
  ).length;
  const fee =
    m.stream.feeCad != null
      ? `Fee CAD $${m.stream.feeCad.toLocaleString()}`
      : 'Fee varies';
  const time =
    m.stream.indicativeProcessingMonths != null
      ? ` · ~${m.stream.indicativeProcessingMonths} mo`
      : '';
  return (
    <>
      <div className="pnp-card-meta">
        <span>
          {met} met{cond > 0 ? ` · ${cond} to secure` : ''}
        </span>
        <span>
          {fee}
          {time}
      ...
      <div className="pnp-nextmove">
        <span className="pnp-nextmove-arrow">→</span> {nextMove(m)}
      </div>
    </>
  );
}
...
function documentsFor(m: PnpStreamMatch, nocCode: string): string[] {
  ...
  if (c.ecaRequired)
    docs.push(
      'Educational Credential Assessment (ECA) from a designated body (e.g. WES)',
    );
  if (c.minSettlementFundsCad)
    docs.push(
      `Proof of settlement funds — at least CAD $${c.minSettlementFundsCad.toLocaleString()}`,
    );
  ...
  if (c.jobOfferRequired === 'required')
    docs.push(`A job offer from an eligible employer in ${m.stream.province}`);
  return docs;
}
  ...
  return (
    <div className="pnp-guide">
      <div className="pnp-guide-head">
        <div>
          <div className="pnp-guide-name">
            {s.province} — {s.streamName}
          </div>
          <div className="pnp-guide-sub">
            {s.programName} ·{' '}
            {s.category === 'ee-linked'
      ...
      {m.conditionalRequirements.length > 0 && (
        <>
          <div className="pnp-subhead">
            Before you apply — secure these first
          </div>
      ...
      <ul className="pnp-prep">
        {documentsFor(m, nocCode).map((d, i) => (
          <li key={i}>{d}</li>
        ))}
      </ul>
      ...
      <div className="pnp-guide-foot">
        <span>
          <b>Processing:</b> {s.processingTimeNote}
        </span>
        {s.feeCad != null && (
          <span>
            <b>Fee:</b> CAD ${s.feeCad.toLocaleString()}
          </span>
        )}
        <span>
          <b>Official source:</b>{' '}
  ...
  return (
    <div className="pnp-pathway">
      <div className="pnp-pathway-top">
        <span
          className={`pnp-pathway-badge pnp-pathway-badge--${open ? 'open' : 'closed'}`}
        >
          {open ? 'Points path open' : 'Points path closed'}
        </span>
        <span className="pnp-pathway-noc">
          NOC {pathway.nocCode} · TEER {pathway.teer}
        </span>
      </div>
      ...
      <div className="pnp-pathway-sectors">
        <div className="pnp-pathway-sector pnp-pathway-sector--priority">
          <div className="pnp-pathway-sector-head">
            Priority sectors · continuous intake
          </div>
          <p className="pnp-pathway-sector-body">
            {prioritySectors.sectors.join(', ')}.{' '}
            {prioritySectors.allocationNote} These candidates may apply at any
            time, including from overseas.
          </p>
        </div>
        <div className="pnp-pathway-sector pnp-pathway-sector--capped">
          <div className="pnp-pathway-sector-head">
            Capped sectors · {cappedSectors.intakeWindows2026.length} intake
            windows (EPA)
          </div>
          <ul className="pnp-pathway-cap-list">
            {cappedSectors.sectors.map((s) => (
              <li key={s.name}>
                <b>{s.name}</b> — {s.capPct}% ({s.capNominations})
              </li>
            ))}
          </ul>
          <p className="pnp-pathway-sector-body">
            {cappedSectors.intakeWindows2026.join(' · ')}.{' '}
            {cappedSectors.allocationNote}
          </p>
        </div>
      </div>

      <p className="pnp-pathway-note">
        Sector membership is set by the employer&rsquo;s job at Employer
        Position Assessment (EPA) time, not by a public NOC list, so the sector
        model above is shown as context. Source: saskatchewan.ca · verified{' '}
        {sinp2026.lastVerified}.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="pnp-sinp">
      <div className="pnp-sinp-top">
        <div>
          <div className="pnp-sinp-total">
            {sinp.computedPoints}
            <span className="pnp-sinp-max">/{sinp.maxPoints}</span>
          </div>
          <div className="pnp-sinp-sub">
            Estimated SINP points — Factor I (Labour Market Success) computed
            from this profile
          </div>
        </div>
        <div
          className={`pnp-sinp-verdict pnp-sinp-verdict--${sinp.meetsPassMark ? 'pass' : 'below'}`}
        >
          {sinp.meetsPassMark
      ...
      <div className="pnp-sinp-track">
        ...
        <div
          className="pnp-sinp-mark"
          style={{ left: `${markPct}%` }}
          aria-label={`Pass mark ${sinp.passMark}`}
        />
      </div>
      <div className="pnp-sinp-rows">
        {sinp.factors.map((f) => (
          <div key={f.key} className="pnp-sinp-row" title={f.detail}>
            ...
            <span
              className={`pnp-sinp-tag pnp-sinp-tag--${f.status === 'to-confirm' ? 'confirm' : 'counted'}`}
            >
              {f.status === 'to-confirm' ? 'to confirm' : 'counted'}
            </span>
            <span className="pnp-sinp-pts">
              {f.points}
              <span className="pnp-sinp-pts-max">/{f.maxPoints}</span>
            </span>
          </div>
        ))}
      </div>
      {sinp.hasUnconfirmedFactors && (
        <p className="pnp-sinp-note">
          Items marked &ldquo;to confirm&rdquo; are not held in this profile —
          chiefly the Saskatchewan connection (Factor II) and any second
          official-language test. Confirming them can only raise the total. The
          SINP minimum eligibility score is {sinp.passMark} of {sinp.maxPoints}.
        </p>
      )}
    </div>
  );
...
function keyCondition(m: PnpStreamMatch): string {
  ...
  return (
    m.conditionalRequirements[0]?.replace(/\.$/, '') ?? `Verdict: ${m.verdict}`
  );
}
...
function fitLabel(m: PnpStreamMatch): string {
  switch (m.occupationEligibility) {
    case 'eligible-listed':
      return 'On list';
    case 'unknown':
      return 'List unverified';
    case 'conditional-employer':
      return 'Employer-set';
    default:
      return m.relevance === 'targeted' ? 'Field match' : 'Open';
  }
}
...
function SinpDrawsCard({
  analysis,
}: {
  analysis: SinpDrawAnalysis;
}): React.JSX.Element {
  ...
  return (
    <div className="pnp-draws">
      ...
      <div className="pnp-draws-summary">
        <span className="pnp-draws-chip pnp-draws-chip--clears">
          {analysis.clears} above
        </span>
        <span className="pnp-draws-chip pnp-draws-chip--conditional">
          {analysis.conditional} conditional
        </span>
        <span className="pnp-draws-chip pnp-draws-chip--out-of-range">
          {analysis.outOfRange} below reach
        </span>
      </div>
      ...
      <div className="pnp-draws-table" role="table">
        {analysis.comparisons.map((c) => (
          <div className="pnp-draws-row" role="row" key={c.date}>
            ...
            <span className="pnp-draws-cutoff">
              <b>{c.cutoffScore}</b> cutoff
            </span>
            <span className="pnp-draws-invited">
              <b>{c.invitationsIssued}</b> invited
            </span>
            <span
              className={`pnp-draws-verdict pnp-draws-verdict--${c.verdict}`}
            >
      ...
      <p className="pnp-draws-note">
        &ldquo;Standing&rdquo; compares this profile&rsquo;s band against
        historical cutoffs only. It is not a prediction, probability, or
        guarantee of an invitation; SINP draw activity is intermittent and
        cutoffs change without notice.
      </p>
      <p className="pnp-draws-source">
        Draw data: saskatchewan.ca EOI Selection Results · last updated{' '}
        {formatDrawDate(analysis.lastUpdated)}
      </p>
    </div>
  );
}

export default function PnpReport({
  profile,
  pnp,
  onBack,
  onDownload,
}: PnpReportProps): React.JSX.Element {
  const { noc } = pnp;
  ...
  const reportGenerated = formatDrawDate(
    profile.reportDate || new Date().toISOString().slice(0, 10),
  );
    ...
    <div className="pnp">
      ...
      <header className="pnp-header">
        <div className="pnp-header-inner">
          ...
          <div className="pnp-prepared">
            Prepared for: <strong>{profile.name || 'Applicant'}</strong>
          </div>
          <div className="pnp-meta">
            NOC {noc.nocCode} · TEER {noc.teer} ·{' '}
            {titleCaseOccupation(noc.title)}
          </div>
          <div className="pnp-meta">
            Report generated: {reportGenerated} · Stream data verified:{' '}
            {reportGenerated} · Source: canada.ca
          </div>
          <div className="pnp-toolbar">
            <button className="pnp-btn" onClick={onBack}>
              ← Back to form
            </button>
            <button className="pnp-btn" onClick={() => window.print()}>
              Print / Save PDF
            </button>
            <button className="pnp-btn pnp-btn--primary" onClick={onDownload}>
      ...
      <div className="pnp-inner">
        ...
        <section className="pnp-section">
          <SectionHead
            eyebrow="Occupation Classification"
            title="Closest NOC match"
          />
          <div className="pnp-noc-winner">
            <div className="pnp-noc-code">NOC {noc.nocCode}</div>
            <div className="pnp-noc-title">
              {titleCaseOccupation(noc.title)}
            </div>
            <div className="pnp-tags">
              ...
              <span
                className={`pnp-chip ${noc.verified ? 'pnp-chip--verified' : 'pnp-chip--unverified'}`}
              >
                {noc.verified
                  ? '✓ Verified on Statistics Canada'
                  : 'Grounded in StatCan data'}
              </span>
            </div>
            <div className="pnp-cite">
              <a
                href={noc.citationUrl}
                target="_blank"
                rel="noopener noreferrer"
              >
                View the official occupational profile ↗
              </a>
            </div>
          </div>
          ...
          {noc.nocOverrideConflict && (
            <div className="pnp-callout pnp-callout--override">
              ...
              <p>
                ...
                <strong>NOC {noc.nocOverrideConflict.yourSelection}</strong> was
                overridden. The duties classifier identified{' '}
                <strong>NOC {noc.nocOverrideConflict.correctedTo}</strong> as
                the closer match based on the applicant&rsquo;s documented
                duties. Review the occupation profile above and correct the NOC
                in the form if the classifier is wrong.
              </p>
            </div>
          )}

          <div className="pnp-occprofile">
            <span className="pnp-occprofile-label">
              Broad Occupational Category
            </span>
            <span className="pnp-occprofile-val">
              {pnp.occupationProfile.broadCategory} —{' '}
              {pnp.occupationProfile.broadCategoryName}
            </span>
            <a
              className="pnp-occprofile-cite"
              href={pnp.occupationProfile.broadCategoryUrl}
              target="_blank"
              rel="noopener noreferrer"
            >
            ...
            <>
              ...
              <div className="pnp-candidates">
                {noc.candidates.map((c, i) => (
                  <div
                    key={c.nocCode}
                    className={`pnp-candidate ${i === 0 ? 'pnp-candidate--top' : ''}`}
                  >
          ...
          {noc.ambiguity.flag && (
            <div className="pnp-callout">
              <div className="pnp-callout-label">
                NOC ambiguity — confirm before relying on this
              </div>
              <p>
                ...
                {noc.ambiguity.alternatives.length > 0 && (
                  <>
                    {' '}
                    (
                    {noc.ambiguity.alternatives
                      .map((a) => `${a.nocCode} TEER ${a.teer}`)
                      .join(', ')}
                    )
                  </>
                )}
                . Confirm the code against the employment reference letter — a
                wrong NOC is the single highest-frequency PR refusal trigger.
              </p>
            </div>
          )}
        </section>
        ...
        <section className="pnp-section">
          ...
          <p className="pnp-lead">
            {profile.name || 'The applicant'}&rsquo;s documented duties classify
            to{' '}
            <strong>
              NOC {noc.nocCode} (TEER {noc.teer})
            </strong>
            , {titleCaseOccupation(noc.title)}. Of {pnp.sourceLog.length} active
            Provincial and Territorial Nominee streams assessed (Quebec
            excluded), the {pnp.shortlist.length} below are the strongest, most
            occupation-relevant pathways for this profile.
          </p>
          <p className="pnp-lead">
            {topEe
        ...
        <section className="pnp-section">
          <SectionHead
            ...
            title={`Your strongest ${pnp.shortlist.length} streams`}
          />
          {pnp.shortlist.length > 0 ? (
            <div
              className={`pnp-cards ${pnp.shortlist.length < 2 ? 'pnp-cards--single' : ''}`}
            >
              {pnp.shortlist.map((m) => (
                <div
                  key={m.stream.id}
                  className={`pnp-card ${m.relevance === 'targeted' ? 'pnp-card--targeted' : ''}`}
                >
                  <div className="pnp-card-top">
                    <div>
                      <div className="pnp-card-name">{m.stream.province}</div>
                      <div className="pnp-card-sub">
                        {m.stream.streamName} ·{' '}
                        {m.stream.category === 'ee-linked'
                          ? 'EE-linked'
                          : 'Base'}
                      </div>
                    </div>
                    <div className="pnp-card-score">
                      {m.score}
                      <span
                        style={{
                          fontSize: '0.75rem',
                  ...
                  {m.occupationEligibility === 'eligible-listed' && (
                    <div className="pnp-occ pnp-occ--listed">
                      On {m.stream.province}&rsquo;s eligible occupation list
                    </div>
                  )}
                  {m.occupationEligibility === 'unknown' && (
                    <div className="pnp-occ pnp-occ--unknown">
                      Occupation list not yet verified — confirm on the
                      provincial source
                    </div>
                  )}
                  {m.occupationEligibility === 'conditional-employer' && (
                    <div className="pnp-occ">
                      Occupation eligibility is set at the job-offer /
                      employer-assessment stage
                    </div>
                  )}
                  <div className="pnp-why">
                    <strong>Why this fits:</strong> {m.whyRelevant}
                  </div>
                  <EligibilityChecks checks={m.eligibilityChecks} />
                  <ScoreBreakdown m={m} />
                  <div className="pnp-score-caption">
                    Eligibility {m.scoreBreakdown.matchStrength} · Strategic
                    priority{' '}
                    {m.scoreBreakdown.strategicValue +
                      m.scoreBreakdown.openStatus +
                      m.scoreBreakdown.processingSpeed}{' '}
                    — ranks fit, not your odds of selection.
                  </div>
                  {m.conditionalRequirements.length > 0 && (
                    <ul className="pnp-conds">
                      {m.conditionalRequirements.map((c, i) => (
                        <li key={i}>
                          <span className="pnp-secure">Secure:</span> {c}
                        </li>
                      ))}
                    </ul>
                  )}
            ...
            <div className="pnp-empty">
              No stream currently matches this profile. Improving language
              scores or securing an in-province job offer typically opens these
              pathways.
            </div>
          )}
        </section>
        ...
        {showSinp && (
          <section className="pnp-section">
            <SectionHead
              ...
              title="SINP 2026 pathway"
              hint="sector-based selection"
            />
            <SinpPathwayCard pathway={sinpPathway} />

            <SectionHead
              eyebrow="Saskatchewan · SINP"
              title="SINP points estimate"
              hint={`pass mark ${sinp.passMark}/${sinp.maxPoints}`}
            />
            <SinpCard sinp={sinp} />
            ...
            {sinpDraws.comparisons.length > 0 && (
              <details className="pnp-draws-legacy">
                <summary className="pnp-draws-legacy-summary">
                  <span className="pnp-draws-legacy-tag">
                    Historical reference
                  </span>
                  Standing vs last 5 EOI draws — points-draw system is dormant
                  (last draw {formatDrawDate(sinpDraws.lastUpdated)}). Click to
                  expand.
                </summary>
        ...
        {pnp.shortlist.length > 0 && (
          <section className="pnp-section">
            <SectionHead
              ...
              title="Step-by-step, per stream"
            />
            {pnp.shortlist.map((m) => (
              <ApplicationGuide key={m.stream.id} m={m} nocCode={noc.nocCode} />
            ))}
          </section>
        )}
        ...
        <section className="pnp-section">
          <SectionHead
            eyebrow="Reference"
            title="Ranked pathways"
            hint="every eligible stream"
          />
              ...
              <tbody>
                {pnp.rankedPathways.map((m) => (
                  <tr key={m.stream.id}>
                    ...
                    <td>
                      <div>{m.stream.streamName}</div>
                      <DifficultyChips stream={m.stream} />
                    </td>
                    <td>
                      {m.stream.category === 'ee-linked' ? 'EE-linked' : 'Base'}
                    </td>
                    <td>{fitLabel(m)}</td>
          ...
          <div className="pnp-caption">
            {eligible.length} streams match both this profile&rsquo;s attributes
            and occupation field. {pnp.ineligible.length} excluded for a hard
            requirement gap
            {fieldExcluded > 0
              ? `; ${fieldExcluded} excluded as locked to a different occupation field`
              : ''}
            . Stream data verified {pnp.dataVersion}.
          </div>
        </section>
        ...
        <div className="pnp-disclaimer">
          ...
          <p>
            ...
            constitute legal advice, and no solicitor-client or
            consultant-client relationship is created by accessing this content.
            Immigration regulations, program requirements, processing times, and
            CRS cutoff scores are subject to frequent change without notice. You
            are responsible for verifying all information with official IRCC
            sources (www.canada.ca/immigration) and confirming current
            eligibility requirements before taking any action.
          </p>
        </div>

        <div className="pnp-footer">
          visaforte.com · prashant@visaforte.com · Secunderabad, India
          <br />
