...
export default function CrmTable({
  ...
  serviceTiers,
  initialDocCounts,
  initialUnreadFromClient,
  oldestUnreadClientMsgTs,
  resumeMap,
}: Props) {
  ...
  const [docCounts, setDocCounts] =
    useState<Record<string, number>>(initialDocCounts);
  ...
  const [unreadFromClient, setUnreadFromClient] = useState<
  ...
  function handleUndoStage(): void {
    ...
    setClients((prev) =>
      prev.map((c) => (c.id === clientId ? { ...c, stage: oldStage } : c)),
    );
    setPendingStage(null);
  }
    ...
    try {
      for (const file of uploadFiles) {
        ...
        const res = await fetch(
          `/api/admin/clients/${docsModal.clientId}/documents`,
          {
            method: 'POST',
            body: form,
          },
  ...
  async function handleDeleteDoc(docId: string) {
    ...
    try {
      await fetch(
        `/api/admin/clients/${docsModal.clientId}/documents/${docId}`,
        {
          method: 'DELETE',
        },
      );
  ...
  async function handleLinkPortal(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setLinkError(
          'Could not link the portal account. Check the email and try again.',
        );
        return;
      }
  ...
  async function openMsgModal(clientId: string, clientName: string) {
    ...
    try {
      ...
      if (
        ...
        fetch(`/api/admin/clients/${clientId}/messages/read`, {
          method: 'PATCH',
        }).catch(() => {});
  ...
  async function handleDownloadTranscript() {
    ...
    try {
      const res = await fetch(
        `/api/admin/clients/${msgModal.clientId}/messages/transcript`,
      );
  ...
  function handleDownloadMsgAttachment(msgId: string, clientId: string) {
    window.open(
      `/api/admin/clients/${clientId}/messages/${msgId}/attachment`,
      '_blank',
    );
  }
        ...
        <div className="crm-ita-banner">
          ...
          <span className="crm-ita-banner-text">
            ...
            <span className="crm-ita-names-trigger">
              {itaClients.length}{' '}
              {itaClients.length === 1 ? 'client' : 'clients'}
              <span className="crm-ita-names-tooltip" aria-hidden="true">
                {itaClients.map((c) => (
                  <span key={c.id} className="crm-ita-names-tooltip-item">
                    {c.name}
                  </span>
                ))}
              </span>
            ...
            <thead>
              <tr>
                {[
                  {
                    label: 'Client',
                    title: 'Name, contact details, and service tier',
                  },
                  {
                    label: 'Stage',
                    title:
                      'Current pipeline stage — ITA Window has a 12-hour response SLA',
                  },
                  { label: 'Added', title: 'Date added to CRM' },
                  {
                    label: 'Notes',
                    title: 'Internal notes — visible only to admin',
                  },
                  {
                    label: 'Msg',
                    title:
                      'Message thread — saffron border means unread client message',
                  },
                  {
                    label: 'Docs',
                    title: 'Client documents and assessment resume',
                  },
                  {
                    label: 'Actions',
                    title: 'Portal link, CanDoc AI review, and delete',
                  },
                ].map((h) => (
                  ...
                  <tr key={client.id} className={isIta ? 'crm-row-ita' : ''}>
                    ...
                    <td>
                      ...
                      <a
                        ...
                        className="crm-td-client-email"
                      >
                        {client.email}
                      </a>
                      <span
                        className="crm-td-client-tier"
                        title={client.serviceTier}
                      >
                        {client.serviceTier}
                      </span>
                    </td>
                    ...
                    <td>
                      <div className="crm-stage-cell">
                        <select
                          ...
                          onChange={(e) =>
                            handleStageChange(client.id, e.target.value)
                          }
                        >
                          {CRM_STAGES.map((s) => (
                    ...
                    <td className="crm-msg-col">
                      <div className="crm-msg-cell">
                        {slaBreached && (
                          <span
                            className="crm-sla-warn"
                            title="Unanswered client message past SLA"
                          >
                            ⚠
                          </span>
                        )}
                        <button
                          ...
                          aria-label={
                            hasUnread
                              ? `Unread message from ${client.name}`
                              : `Message ${client.name}`
                          }
                        >
                    ...
                    <td className="crm-actions-col">
                      <div className="crm-actions-cell">
                        <span className="crm-portal-slot">
                          {client.userId ? (
                            ...
                            <button
                              ...
                              onClick={() =>
                                openLinkModal(client.id, client.email)
                              }
                        ...
                        <button
                          ...
                          onClick={() =>
                            router.push(
                              `/admin/candoc?clientId=${client.id}&name=${encodeURIComponent(client.name)}`,
                            )
                          }
            ...
            <form onSubmit={handleAddClient} className="crm-modal-form">
              ...
              <label className="crm-field-label">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, serviceTier: e.target.value }))
                  }
                  required
                >
          ...
          <div className="crm-modal crm-delete-modal">
            ...
            <div className="crm-delete-modal-body">
              <p className="crm-delete-warning">
                ...
                <strong>{deleteTarget.name}</strong> and all their documents.
            ...
            <div className="crm-modal-footer">
              ...
              <button
                ...
                disabled={
                  !deletePassword || deletingClientId === deleteTarget.id
                }
              >
                {deletingClientId === deleteTarget.id
                  ? 'Deleting…'
                  : 'Delete Permanently'}
              </button>
            </div>
            ...
            {linkSuccess ? (
              ...
              <form onSubmit={handleLinkPortal} className="crm-modal-form">
                <p className="crm-link-hint">
                  Enter the email address the client will use to log in. If no
                  account exists, one will be created and an invite email will
                  be sent.
                </p>
                <label className="crm-field-label">
                  Client Email *
          ...
          <div className="crm-modal crm-msg-modal">
            <div className="crm-modal-header">
              ...
              <div className="crm-msg-modal-actions">
                ...
                <button className="crm-modal-close" onClick={closeMsgModal}>
                  ✕
                </button>
              </div>
            </div>
                  ...
                  >
                    ...
                    {m.attachmentUrl && (
                      <button
                        ...
                        onClick={() =>
                          handleDownloadMsgAttachment(m.id, msgModal.clientId)
                        }
                      >
                        ↓ Attachment
                      </button>
            ...
            <form className="crm-msg-compose" onSubmit={handleSendMessage}>
              ...
              <div className="crm-msg-attach-row">
                <input
                  ...
                  onChange={(e) =>
                    setMsgAttachFile(e.target.files?.[0] ?? null)
                  }
                />
                <label
                  htmlFor="crm-msg-attach-input"
                  className="crm-msg-attach-label"
                >
          ...
          <div className="crm-modal crm-docs-modal">
            ...
            <div className="crm-docs-body">
              ...
              <div className="crm-upload-area">
                ...
                <p className="crm-upload-hint">
                  PDF, Word, Excel, or image · Max 20 MB · Select multiple files
                  at once
                </p>
              </div>
