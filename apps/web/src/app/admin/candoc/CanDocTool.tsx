...
export default function CanDocTool(): React.JSX.Element {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      if (
        ['analyzed', 'annotating', 'complete', 'error', 'none'].includes(
          data.status,
        )
      ) {
        if (pollRef.current) {
  ...
  const handleTrigger = async (): Promise<void> => {
    ...
    try {
      ...
      const { reviewId: newId, error: trigErr } = (await trigRes.json()) as {
        reviewId?: string;
        error?: string;
      };
      if (trigErr || !newId) {
        setError(trigErr ?? 'Trigger failed');
        return;
      }
  ...
  return (
    <div className="candoc-wrap">
      <div className="candoc-header">
        ...
        <span className={`candoc-status-badge ${status}`}>
          {STATUS_LABEL[status]}
        </span>
      </div>
      ...
      {status === 'analyzing' && (
        <p style={{ color: 'var(--clr-text-secondary)', marginTop: '1rem' }}>
          Claude is reviewing all documents across 17 SOP layers. This takes up
          to 2 minutes…
        </p>
      )}
...
function FindingsView({
    ...
    try {
      const res = await fetch('/api/admin/candoc/signoff', {
        ...
        body: JSON.stringify({
          ...
          annotatedFindings: local,
        }),
      });
      const data = (await res.json()) as { ok?: boolean; error?: string };
      if (data.error) {
        setSignoffError(data.error);
        return;
      }
  ...
  return (
    <div>
      ...
      >
        <span
          style={{ fontSize: '0.875rem', color: 'var(--clr-text-secondary)' }}
        >
          Reviewed: {new Date(findings.reviewedAt).toLocaleString()} · v
          {findings.version} · Risk:{' '}
          <strong>{findings.overallRiskLevel.toUpperCase()}</strong> · Gaps:{' '}
          {findings.totalGaps}
        </span>
        {status !== 'complete' && (
          <button
        ...
        {local.sopLayers.map((layer, li) => (
          <div key={layer.layer} className="candoc-layer-card">
            <div className="candoc-layer-header">
              <span className="candoc-layer-title">
                {layer.layer} — {layer.layerName}
              </span>
              <span className={`candoc-layer-badge ${layer.status}`}>
                {layer.status}
              </span>
            </div>
            ...
            {layer.findings.map((finding, fi) => (
              <div
                key={finding.id}
                className={`candoc-finding ${finding.severity}${finding.isResolved ? ' resolved' : ''}`}
              >
                {finding.isNew && (
                  <span
                    style={{
