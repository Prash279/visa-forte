...
function parseResidenceCountries(form: FormValues): string[] {
  const countries = form.residenceCountries
    .split(',')
    .map((c) => c.trim())
    .filter((c) => c.length > 0);
...
export default function ItaCountdownTool({ initialToken }: Props) {
  ...
  const [errorMessage, setErrorMessage] = useState(
    initialToken === null ? 'Link expired or invalid.' : '',
  );
  ...
  const fetchResult = useCallback(async (token: string) => {
    try {
      const res = await fetch(
        `/api/tools/ita-countdown/result?token=${encodeURIComponent(token)}`,
      );
      if (!res.ok) {
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: {
          ...
          try {
            ...
            setErrorMessage(
              'Payment was received but we could not load your checklist. Please email prashant@visaforte.com with your payment ID.',
            );
            setState('error');
          }
          resolve();
        ...
        <section className="asx-form-section">
          <div className="asx-form-inner itc-result">
            <p className="itc-result-intro">
              Dear {result.name}, here is your personalised document checklist
              based on an ITA date of <strong>{result.itaDate}</strong>.
            </p>
            <p className="itc-email-note itc-no-print">
              ✓ A copy has also been emailed to you.
            </p>

            <div className="itc-checklist">
              {result.checklist.map((item) => {
                const daysToDeadline = daysBetween(
                  result.itaDate,
                  item.deadlineDate,
                );
                return (
                  <div
                    key={item.id}
                    className={`itc-card${daysToDeadline <= 10 ? ' itc-card--urgent' : ''}`}
                  >
                    <h3 className="itc-card-task">{item.task}</h3>
                    <div className="itc-card-dates">
                      <span>
                        Start by <strong>{item.startByDate}</strong>
                      </span>
                      <span>
                        Deadline <strong>{item.deadlineDate}</strong>
                      </span>
                    </div>
          ...
          <div className="asx-form-inner">
            ...
            {initialToken === undefined ? (
              ...
              <a
                ...
                className="asx-submit-btn itc-link-btn"
              >
  ...
  if (state === 'sample') {
    ...
    if (form.numDependentChildren > 0) {
      profileParts.push(
        `${form.numDependentChildren} dependent ${form.numDependentChildren > 1 ? 'children' : 'child'}`,
      );
    }
        ...
        <section className="asx-hero">
          <div className="asx-hero-inner">
            ...
            <h1 className="asx-hero-headline r d1">
              Your Checklist. Dates Locked.
            </h1>
            <div className="rule r d2" />
            <p className="asx-hero-lead r d2">
              All {previewChecklist.length} tasks below are generated from your
              profile ({profileDescription}) for an ITA date of{' '}
              {formatDate(form.itaDate)}. Purchase to unlock the exact start-by
              and deadline date for every task.
            </p>
          </div>
        </section>
        <section className="asx-form-section">
          <div className="asx-form-inner">
              ...
              {previewChecklist.map((item) => (
                <div key={item.id} className="itc-card itc-card--sample">
                  ...
                  <div className="itc-card-dates">
                    <span>
                      Start by{' '}
                      <span className="itc-locked-value">•• •••, ••••</span>
                    </span>
                    <span>
                      Deadline{' '}
                      <span className="itc-locked-value">•• •••, ••••</span>
                    </span>
                    <span className="itc-locked-tag">
            ...
            <div className="asx-submit-row">
              ...
              <p className="asx-submit-note">
                Secure payment via Razorpay. Your personalised checklist is
    ...
    <div className="asx-wrap">
      <section className="asx-hero">
        <div className="asx-hero-inner">
          ...
          <div className="rule r d2" />
          <p className="asx-hero-lead r d2">
            Enter your ITA date and get a personalised day-by-day document
            preparation timeline — printable and emailed to you.
          </p>
        </div>
      </section>
        ...
        <div className="asx-form-inner">
          <form onSubmit={handleFormSubmit} noValidate>
            <div className="asx-grid-2">
              <div className="asx-field">
                <label className="asx-label" htmlFor="itc-name">
                  Full Name
                </label>
                <input
                  className="asx-input"
              ...
              <div className="asx-field">
                <label className="asx-label" htmlFor="itc-email">
                  Email
                </label>
                <input
                  className="asx-input"
            ...
            <div className="asx-grid-2">
              <div className="asx-field">
                <label className="asx-label" htmlFor="itc-ita-date">
                  ITA Date
                </label>
                <input
                  className="asx-input"
                  ...
                  value={form.itaDate}
                  onChange={(e) =>
                    setForm({ ...form, itaDate: e.target.value })
                  }
                />
              </div>
              <div className="asx-field">
                <label className="asx-label" htmlFor="itc-citizenship">
                  Citizenship Country
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, citizenshipCountry: e.target.value })
                  }
                >
            ...
            <div className="asx-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, residenceCountries: e.target.value })
                }
              />
            </div>
            ...
            <div className="asx-grid-2">
              <div className="asx-field">
                <span className="asx-label itc-label-spacer" aria-hidden="true">
                  &nbsp;
                </span>
                <label className="asx-checkbox-row">
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, hasSpouse: e.target.checked })
                    }
                  />
              ...
              <div className="asx-field">
                <label className="asx-label" htmlFor="itc-children">
                  Dependent Children
                </label>
                <select
                  className="asx-select"
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      numDependentChildren: Number(e.target.value),
                    })
                  }
                >
                  {[0, 1, 2, 3, 4, 5].map((n) => (
            ...
            <div className="itc-tier-pills">
              ...
              >
                Premium — ₹3,997{' '}
                <span className="itc-tier-pill-note">+ doc review call</span>
              </button>
            </div>
          ...
          <div className="asx-disclaimer">
            ...
            <p className="asx-disclaimer-body">
              The information provided in this tool is for informational and
              ...
              This does not constitute legal advice, and no solicitor-client or
              consultant-client relationship is created by using this tool.
              Document processing times vary by country and can change without
              notice — verify current timelines with the issuing authority and
              with official IRCC sources (www.canada.ca/immigration).
            </p>
          </div>
        </div>
      </section>
