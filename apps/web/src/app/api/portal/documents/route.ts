...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!client) {
    return NextResponse.json({ error: 'No linked client record found' }, { status: 404 })
  }
  ...
  if (!validIds.has(docType)) {
    return NextResponse.json({ error: 'Invalid document type for this service tier' }, { status: 400 })
  }
  ...
  let blobUrl: string
  try {
    const result = await uploadFile(blobPathname, buffer, file.type || 'application/octet-stream')
    blobUrl = result.url
  } catch (err) {
