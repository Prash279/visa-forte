...
import type { CrsResult, ApplicantProfile, StreamEligibility, LanguageBands } from './crs-calculator'
...
const CATEGORY_MATCHERS: Array<{ category: string; test: (t: string) => boolean }> = [
...
function latestDraw(category: string): DrawEntry | null {
  ...
  return (drawData.draws as DrawEntry[]).find(d => matcher.test(d.type)) ?? null
}
...
export function getBestPathway(
  ...
  const candidates = eligibleCategories
    .map(cat => {
      ...
      if (!draw) return null
      return { category: cat, cutoffScore: draw.cutoffScore, drawDate: draw.date, gap: crsScore - draw.cutoffScore }
    })
