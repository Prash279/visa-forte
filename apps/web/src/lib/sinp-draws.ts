...
export function drawVerdict(bandFloor: number, bandCeiling: number, cutoffScore: number): SinpDrawVerdict {
...
function groupByDate(draws: SinpDraw[]): Omit<SinpDrawComparison, 'verdict'>[] {
  ...
  for (const draw of draws) {
    ...
    if (!existing.subCategories.includes(draw.subCategory)) existing.subCategories.push(draw.subCategory)
...
export function analyzeSinpDraws(
  ...
  const comparisons: SinpDrawComparison[] = groupByDate(dataset.draws).map((group) => ({
    ...group,
    verdict: drawVerdict(floor, ceiling, group.cutoffScore),
  }))

  const count = (v: SinpDrawVerdict): number => comparisons.filter((c) => c.verdict === v).length
