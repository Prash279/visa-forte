...
function nextMove(m: PnpStreamMatch): string {
  if (m.verdict === 'confirmed')
    return 'Meets every checkable requirement — proceed to documentation.';
...
function titleSlide(
  ...
  s.addText('Engineered for Passage.', {
    x: 0.8,
    y: 1.3,
    w: 8,
    h: 0.35,
  ...
  s.addText('PNP Pathway Assessment', {
    x: 0.8,
    y: 2.3,
    w: 11.5,
    h: 1.1,
  ...
  s.addText(
    [
      ...
      {
        text: profile.name || 'Applicant',
        options: { color: PEARL, bold: true },
      },
    ],
    { x: 0.8, y: 3.7, w: 11, h: 0.4, fontFace: SANS, fontSize: 15 },
  );
  s.addText(
    `NOC ${pnp.noc.nocCode} · TEER ${pnp.noc.teer} · ${titleCaseOccupation(pnp.noc.title)}`,
    {
      x: 0.8,
      y: 4.1,
      w: 11.5,
...
function nocSlide(
  ...
  if (pnp.noc.candidates.length > 1) {
    s.addText('Ranked matches considered', {
      x: 0.8,
      y: 2.8,
      w: 8,
      h: 0.3,
    ...
    const rows = pnp.noc.candidates.slice(0, 4).map((c, i) => [
      ...
      {
        text: `NOC ${c.nocCode} (TEER ${c.teer})`,
        options: { color: PRUSSIAN, bold: true, valign: 'middle' },
      },
      {
        text: titleCaseOccupation(c.title),
        options: { color: INK, valign: 'middle' },
      },
    ...
    s.addTable(rows, {
      ...
      y: 3.15,
      w: W - 1.6,
      colW: [0.5, 2.2, 3.0, 6.0],
      fontFace: SANS,
      fontSize: 11,
      color: INK,
      valign: 'middle',
      border: { type: 'solid', color: SAND, pt: 1 },
      rowH: 0.4,
      autoPage: false,
    });
  }
...
function pathwaysSlide(
  ...
  sectionHeader(
    ...
    `Your strongest ${pnp.shortlist.length} streams`,
  );
  ...
  const rows = pnp.shortlist.map((m) => [
    ...
    {
      text: m.stream.category === 'ee-linked' ? 'EE-linked' : 'Base',
      options: {
        color: m.stream.category === 'ee-linked' ? SAFFRON : TEAL,
        valign: 'middle',
      },
    },
    {
      text: VERDICT_LABEL[m.verdict],
      options: {
    ...
    {
      text: nextMove(m),
      options: { color: INK, fontSize: 10, valign: 'middle' },
    },
  ]);
  ...
  s.addText(
    'Score ranks fit and strategic value — not the probability of selection.',
    {
      x: 0.8,
      y: 6.55,
      w: W - 1.6,
...
function matrixSlide(
  ...
  const eligible = [...pnp.eeLinked, ...pnp.base].filter(
    (m) => m.relevance !== 'mismatch',
  );
  const header = ['Province', 'Stream', 'Category', 'Verdict', 'Score'].map(
    (h) => ({
      text: h,
      options: {
        fill: { color: PRUSSIAN },
  ...
  const rows = eligible.slice(0, 14).map((m) => [
    ...
    {
      text: m.stream.category === 'ee-linked' ? 'EE-linked' : 'Base',
      options: { color: MUTED, valign: 'middle' },
    },
    {
      text: VERDICT_LABEL[m.verdict],
      options: {
...
function sinpPathwaySlide(
  ...
  s.addText(
    [
      {
        text: pathway.pointsPathOpen
          ? 'POINTS PATH OPEN — '
          : 'POINTS PATH CLOSED — ',
        options: { bold: true, color: pathway.pointsPathOpen ? GREEN : AMBER },
      },
  ...
  s.addText('PRIORITY SECTORS · CONTINUOUS INTAKE', {
    x: 1.0,
    y: 2.95,
    w: colW - 0.4,
    h: 0.3,
  ...
  s.addText(
    `CAPPED SECTORS · ${cappedSectors.intakeWindows2026.length} INTAKE WINDOWS (EPA)`,
    {
      x: cappedX + 0.2,
      y: 2.95,
      w: colW - 0.4,
  ...
  const cappedLines = cappedSectors.sectors
    .map((c) => `• ${c.name} — ${c.capPct}% (${c.capNominations})`)
    .join('\n');
  s.addText(
...
function disclaimerSlide(
  ...
  s.addText(
    `PNP Pathway Assessment · Generated ${profile.reportDate} · Stream data verified ${pnp.dataVersion}`,
    {
      x: 0.8,
      y: 6.7,
      w: 8,
  ...
  s.addText(`Visa Forte · ${FOOTER_CONTACT}`, {
    x: W - 5.3,
    y: 6.7,
    w: 4.7,
    h: 0.3,
...
export async function buildPnpPptxBlob(
  profile: ApplicantProfile,
  pnp: PnpAssessmentResult,
): Promise<Blob> {
