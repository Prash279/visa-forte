...
describe('generateChecklist', () => {
  ...
  it('uses the standard note for countries without a long lead time', () => {
    const items = generateChecklist({ ...baseInput, citizenshipCountry: 'UK', residenceCountries: ['UK'] })
    const policeCert = items.find((i) => i.id === 'police_certificate')!
  ...
  it('every item has startByDate on or before deadlineDate', () => {
    const items = generateChecklist({ ...baseInput, hasSpouse: true, numDependentChildren: 1 })
    for (const item of items) {
      expect(item.startByDate.localeCompare(item.deadlineDate)).toBeLessThanOrEqual(0)
    }
  })
