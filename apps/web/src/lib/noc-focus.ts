...
export function nocFocusCategories(code: string): string[] {
  ...
  if (p2 === '31' || p2 === '32' || p2 === '33' || code === '41404') return ['health']
  // Applied sciences & IT, plus computer/network technicians.
  if (p2 === '21' || ['22220', '22221', '22222'].includes(code)) return ['tech']
  // Construction is a subset of the trades.
  if (p3 === '723' || code.startsWith('7231') || code.startsWith('7611')) return ['trades', 'construction']
  ...
  if (p2 === '82' || p2 === '84' || p2 === '85') return ['agriculture', 'natural-resources']
...
export function streamRelevance(streamFocus: string[] | undefined, nocCode: string): StreamRelevance {
  ...
  return streamFocus.some((f) => nocFocus.includes(f)) ? 'targeted' : 'mismatch'
}
