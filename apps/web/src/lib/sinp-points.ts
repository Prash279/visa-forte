...
export function scoreSinp(profile: ApplicantProfile): SinpScore {
  ...
  factors.push({
    ...
    status: 'computed',
    detail:
      'Assumes the recorded experience falls within the 5 years before applying. Additional points for work 6–10 years prior require dated history (to confirm).',
  });
  ...
  if (profile.hasSecondLanguage && profile.secondLanguageScores) {
    ...
    factors.push({
      ...
      status: 'to-confirm',
      detail:
        'No second official-language test on file. A valid test in the other official language adds up to 10 points.',
    });
  }
  ...
  factors.push({
    ...
    status: 'to-confirm',
    detail:
      'Up to 30 points for a high-skilled Saskatchewan job offer (30), a close SK family relative (20), or past SK work/study (5 each) — none captured by this profile.',
  });
