...
function fromClbPts(
  thresholds: ReadonlyArray<{ readonly minClb: number; readonly points: number }>,
  clb: number
): number {
  for (const t of thresholds) {
...
function fromYearsPts(
  thresholds: ReadonlyArray<{ readonly minYears: number; readonly points: number }>,
  years: number
): number {
...
function ieltsAcToClb(ability: 'L' | 'R' | 'W' | 'S', score: number): number {
  return fromMinThresholds(rules.languageConversion.IELTS_Academic[ability], score)
}
...
function tcfToClb(ability: 'L' | 'R' | 'W' | 'S', score: number): number {
  ...
  if (ability === 'L') return fromMinThresholds(rules.languageConversion.TCF.L, score)
  if (ability === 'R') return fromMinThresholds(rules.languageConversion.TCF.R, score)
...
function agePoints(age: number, hasSpouse: boolean): number {
  ...
  const table = (hasSpouse ? rules.sectionA.ageWithSpouse : rules.sectionA.ageSingle) as Record<string, number>
  return table[String(age)] ?? 0
}

function educationPoints(level: EducationLevel, hasSpouse: boolean): number {
  const table = (hasSpouse ? rules.sectionA.educationWithSpouse : rules.sectionA.educationSingle) as Record<EducationLevel, number>
  return table[level] ?? 0
}

function firstLangPointsPerBand(clb: number, hasSpouse: boolean): number {
  return fromClbPts(
    hasSpouse ? rules.sectionA.firstLanguageWithSpouse : rules.sectionA.firstLanguageSingle,
    clb
  )
}
...
function canadianExpPoints(years: number, hasSpouse: boolean): number {
  return fromYearsPts(
    hasSpouse ? rules.sectionA.canadianExpWithSpouse : rules.sectionA.canadianExpSingle,
    years
  )
}
...
function eduLanguageTransfer(edu: EducationLevel, langBands: LanguageBands): number {
  ...
  const isPostSecondary12 = edu === 'one_year_post_secondary' || edu === 'two_year_post_secondary'
  const isBachelorsPlus   = edu === 'bachelors' || edu === 'two_or_more_degrees' || edu === 'masters' || edu === 'doctoral'
...
function eduCanadianExpTransfer(edu: EducationLevel, canadianYears: number): number {
  ...
  if (isBachelorsPlus)   return cWhole >= 2 ? cfg.bachelorsPlus.cwe2plus   : cfg.bachelorsPlus.cwe1
  if (isPostSecondary12) return cWhole >= 2 ? cfg.postSecondary12.cwe2plus : cfg.postSecondary12.cwe1
  return 0
}
...
function foreignExpCanadianExpTransfer(foreignYears: number, canadianYears: number): number {
  ...
  if (fWhole >= 3) return cWhole >= 2 ? cfg.fwe3plus.cwe2plus : cfg.fwe3plus.cwe1
  if (fWhole >= 1) return cWhole >= 2 ? cfg.fwe1plus.cwe2plus : cfg.fwe1plus.cwe1
  return 0
}
...
function fswLanguagePoints(firstBands: LanguageBands, secondBands?: LanguageBands): number {
  const first =
  ...
  if (secondBands) {
    second = Math.min(
      ...
      [secondBands.listening, secondBands.reading, secondBands.writing, secondBands.speaking]
        .filter(clb => clb >= rules.fsw.secondLanguageMinClb)
        .length
    )
  }
...
function assessStreamEligibility(
  ...
  else if (!fswLangOk) fswReason = `Minimum CLB ${minLangClb} in all four abilities not met.`
  else if (!fswExpOk)  fswReason = 'At least 1 year of qualifying work experience required.'
  else if (!fswGrid.eligible) fswReason = `FSW ${rules.fsw.passmark}-point grid not met (scored ${fswGrid.total}/100).`
  else fswReason = `Scores ${fswGrid.total}/100 on FSW ${rules.fsw.passmark}-point grid — clears the pass mark. Eligible to create profile.`

  // CEC: 1yr authorized Canadian work in TEER 0-3, CLB 7 minimum
  const cecEligible = canadianWhole >= 1 && qualifyingTeer && minFirstLang >= minLangClb
...
function buildScenarios(
  ...
  if (foreignWhole < 2) {
    ...
    if (delta > 0) {
      scenarios.push({
        ...
        currentCrs, projectedCrs: currentCrs + delta, delta, competitive: currentCrs + delta >= 480,
      })
    }
  }
  ...
  if (foreignWhole < 3) {
    ...
    if (delta3 > 0) {
      scenarios.push({
        ...
        currentCrs, projectedCrs: currentCrs + delta3, delta: delta3, competitive: currentCrs + delta3 >= 480,
      })
    }
  }
  ...
  if (langDelta > 0) {
    scenarios.push({
      ...
      currentCrs, projectedCrs: currentCrs + langDelta, delta: langDelta, competitive: currentCrs + langDelta >= 480,
    })
  }
  ...
  if (!profile.hasProvincialNomination) {
    ...
    scenarios.push({
      ...
      currentCrs, projectedCrs: currentCrs + pnpDelta, delta: pnpDelta, competitive: true,
    })
  }
  ...
  if (canadianWhole === 0) {
    ...
    scenarios.push({
      ...
      currentCrs, projectedCrs: currentCrs + cweDelta, delta: cweDelta, competitive: currentCrs + cweDelta >= 480,
    })
  }
...
function buildFswImprovementSuggestions(
  ...
  if (fswGrid.language < 24) {
    ...
    if (gain > 0) {
      suggestions.push({
        ...
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
  ...
  if (Math.floor(profile.canadianWorkExperienceYears) === 0 && fswGrid.adaptability < rules.fsw.adaptabilityMax) {
    const newAdapt = fswAdaptabilityPoints(profile.hasCanadianEducation, profile.hasFamilyInCanada, 1, currentHasJobOffer)
    const gain = newAdapt - fswGrid.adaptability
    if (gain > 0) {
      suggestions.push({
        name: 'Gain Canadian Work Experience',
        action: 'Obtain at least 1 year of authorized work in Canada in a NOC TEER 0–3 occupation — adds 5 FSW adaptability points',
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
        wouldQualify: current + gain >= rules.fsw.passmark,
      })
    }
  ...
  if (!currentHasJobOffer && fswGrid.adaptability < rules.fsw.adaptabilityMax) {
    const newAdapt = fswAdaptabilityPoints(profile.hasCanadianEducation, profile.hasFamilyInCanada, profile.canadianWorkExperienceYears, true)
    const gain = newAdapt - fswGrid.adaptability
    if (gain > 0) {
      suggestions.push({
        name: 'Secure a Valid Job Offer in Canada',
        action: 'Obtain a qualifying arranged employment offer (LMIA-supported or LMIA-exempt) from a Canadian employer — adds 5 FSW adaptability points',
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
        wouldQualify: current + gain >= rules.fsw.passmark,
      })
    }
  ...
  if (!profile.hasCanadianEducation && fswGrid.adaptability < rules.fsw.adaptabilityMax) {
    const newAdapt = fswAdaptabilityPoints(true, profile.hasFamilyInCanada, profile.canadianWorkExperienceYears, currentHasJobOffer)
    const gain = newAdapt - fswGrid.adaptability
    if (gain > 0) {
      suggestions.push({
        name: 'Complete a Canadian Study Program',
        action: 'Study full-time in Canada in a post-secondary program of at least 2 academic years — adds 5 FSW adaptability points',
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
        wouldQualify: current + gain >= rules.fsw.passmark,
      })
    }
  ...
  if (profile.hasSpouse && fswGrid.adaptability < rules.fsw.adaptabilityMax) {
    ...
    if (!spouseAlreadyHasCLB4) {
      const newAdapt = Math.min(rules.fsw.adaptabilityMax, fswGrid.adaptability + rules.fsw.adaptabilityPerFactor)
      const gain = newAdapt - fswGrid.adaptability
      if (gain > 0) {
        suggestions.push({
          name: 'Spouse/Partner Language Test (CLB 4+ in All Four Abilities)',
          action: 'Have your accompanying spouse or partner take an approved language test and achieve CLB 4 in Listening, Reading, Writing, and Speaking — adds 5 FSW adaptability points',
          currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
          wouldQualify: current + gain >= rules.fsw.passmark,
        })
      }
    ...
    if (spouseCwe < 1) {
      ...
      if (gain > 0) {
        suggestions.push({
          ...
          action: 'Your accompanying spouse or partner can earn 5 FSW adaptability points by completing at least 1 year of full-time authorized work in Canada on a valid work permit',
          currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
          wouldQualify: current + gain >= rules.fsw.passmark,
        })
      }
  ...
  if (profile.hasSpouse && fswGrid.adaptability < rules.fsw.adaptabilityMax) {
    ...
    if (gain > 0) {
      suggestions.push({
        ...
        action: 'Your accompanying spouse or partner can earn 5 FSW adaptability points by completing at least 2 academic years of full-time study at a secondary or post-secondary school in Canada',
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
        wouldQualify: current + gain >= rules.fsw.passmark,
      })
    }
  ...
  if (fswGrid.workExperience < 15) {
    ...
    if (gain > 0) {
      suggestions.push({
        ...
        action: 'Continue accruing qualifying work experience to reach the 6-year tier (maximum 15 FSW work experience points)',
        currentFswTotal: current, projectedFswTotal: current + gain, pointsGained: gain,
        wouldQualify: current + gain >= rules.fsw.passmark,
      })
    }
...
export function calculate(profile: ApplicantProfile): CrsResult {
  ...
  if (profile.hasSpouse) {
    if (profile.spouseEducation) {
      const spEduTable = rules.sectionB.spouseEducation as Record<EducationLevel, number>
  ...
  const foreignCanTr  = foreignExpCanadianExpTransfer(profile.foreignWorkExperienceYears, profile.canadianWorkExperienceYears)
  const transferTotal = Math.min(rules.sectionC.maxTotal, eduLangTr + eduCanTr + foreignLangTr + foreignCanTr)

  // Section D — Additional
  const provinceNom    = profile.hasProvincialNomination ? rules.sectionD.provincialNomination : 0
  ...
  const fswEdu   = (rules.fsw.education as Record<EducationLevel, number>)[profile.education] ?? 0
  ...
  const fswGrid: FswGrid = {
    language: fswLang, education: fswEdu, workExperience: fswExp,
    age: fswAge, adaptability: fswAdapt, total: fswTotal,
