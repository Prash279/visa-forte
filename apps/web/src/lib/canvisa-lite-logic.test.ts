...
function makeResult(
  ...
  return {
    ...
    breakdown: {
      ...
      agePoints: 100,
      educationPoints: 126,
      firstLanguagePoints: 124,
      secondLanguagePoints: 0,
      canadianExpPoints: 80,
      spousePoints: 0,
      coreTotal: 430,
      eduLanguageTransfer: 20,
      eduCanadianExpTransfer: 0,
      foreignExpLanguageTransfer: 0,
      foreignExpCanadianExpTransfer: 0,
      transferTotal: 20,
...
describe('getBestPathway', () => {
  it('picks the above-cutoff category with smallest gap', () => {
    ...
    const latestPnp = (
      drawData.draws as { type: string; cutoffScore: number }[]
